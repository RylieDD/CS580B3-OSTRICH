...
12. In cell 3 of the OSTRICH_LLM_Model_no_think jupyter notebook, update the Hugging Face token with a valid token to access the Hugging Face models in line 10: "token = """.
13. In cell 6 of the OSTRICH_LLM_Model_no_think jupyter notebook, update the GitHub PAT with a valid token to access the TalonHub/Community repository in line 3: "token = """.
14. Run each cell in order in the OSTRICH_LLM_Model_no_think jupyter notebook to view the output from the training and validate datasets run on the zero-shot and few-shot models with Non-Thinking mode.
